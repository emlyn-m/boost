...
pub fn test_chunking() {
    ...
    let tx_msg_id = 15;
    let test_blocks = user::User::generate_msg_blocks(&tx_payload_bitvec, tx_is_command, tx_msg_id);
    ...
    for i in 0..n_blocks {
        ...
        let rx_msg_id = block[0..5].load::<u8>();
        assert!(rx_msg_id == tx_msg_id);

        let rx_is_command = block[5..6].load::<u8>();
        if tx_is_command { assert!(rx_is_command == 1); }

        let rx_is_mp = block[6..7].load::<u8>();
        assert!(rx_is_mp == 1);

        let rx_mp_first = block[7..8].load::<u8>();
        match rx_mp_first {
            1 => { assert!(i == 0) },
            0 => { assert!(i != 0); },
        ...
        let rx_block_idx = block[8..16].load::<u8>();
        dbg!(rx_block_idx);
        if i == 0 { assert!((rx_block_idx as usize) + 1 == n_blocks); }
        if i != 0 { assert!((rx_block_idx as usize) != i); }

        let rx_payload = match String::from_utf8(block.into_vec()[2..].to_vec()) {
            Ok(payload) => payload,
